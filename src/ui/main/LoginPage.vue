        ...
        <div class="login-container window-move">
            ...
            <div v-if="loginType === 0" class="qrcode-login-container">
                ...
                <div v-if="loginStatus === 0" class="pending-scan">
                    <p style="font-size: 20px; color: #353535; padding-bottom: 10px">
                        {{ $t('login.desc') }}
                    </p>
                ...
                <div v-if="loginStatus === 2" class="pending-quick-login">
                    <button @click="sendQuickLoginRequest" class="button-confirm">
                        {{ $t('login.login') }}
                    </button>
                    <button @click="cancel" class="button-cancel">
                        {{ $t('login.switch_user') }}
...
export default {
    ...
    methods: {
        ...
        async login() {
            ...
            appServerApi
                .loginWithPCSession(this.appToken)
                .then((data) => {
                    if (data) {
                        switch (data.code) {
                            case 0:
        ...
        onConnectionStatusChange(status) {
            ...
            if (status === ConnectionStatus.ConnectionStatusConnected) {
                ...
                organizationServerApi.login().catch((r) => {
