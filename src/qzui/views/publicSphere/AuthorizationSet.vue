    ...
    <el-drawer v-model="drawer" :title="title" :direction="direction" size="45%" header-class="drawer-header">
      <div class="drawer-content">
        <ul class="select-list" v-show="typeDrawer === 'join'">
          <li v-for="item in joinList" :class="{ activeJoin: item.value === selectValue }" :key="item.value"
            @click="handleSelect(item, 'join')">
            {{ item.name }}
          </li>
        </ul>
        <ul class="select-list" v-show="typeDrawer === 'task'">
          <li v-for="item in taskList" :class="{ activeJoin: item.value === selectValue }" :key="item.value"
            @click="handleSelect(item, 'task')">
            {{ item.name }}
          </li>
        </ul>
        <ul class="select-list" v-show="typeDrawer === 'privateMsg'">
          <li v-for="item in privateList" :class="{ activeJoin: item.value === selectValue }" :key="item.value"
            @click="handleSelect(item, 'privateMsg')">
            {{ item.name }}
          </li>
        </ul>
