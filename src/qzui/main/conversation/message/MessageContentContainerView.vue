<template>
    <div>
        ...
        <AudioMessageContentView :message="message" v-else-if="message.messageContent.type === 23" />
        <ImageMessageContentView :message="message" v-else-if="message.messageContent.type === 3" />
        <!--                           v-bind:class="{leftarrow:message.direction === 1, rightarrow: message.direction === 0}"/>-->
        <FileMessageContentView :message="message" v-else-if="message.messageContent.type === 5" v-bind:class="{ leftarrow: message.direction === 1, rightarrow: message.direction === 0 }" />
        ...
        <UserCardMessageContentView
            ...
            v-bind:class="{ leftarrow: message.direction === 1, rightarrow: message.direction === 0 }"
        />
        <MixMultiMediaTextMessageContentView :message="message" v-else-if="message.messageContent.type === MessageContentType.MESSAGE_CONTENT_TYPE_MIX_MULTI_MEDIA_TEXT" />
        <MixFileTextMessageContentView :message="message" v-else-if="message.messageContent.type === MessageContentType.MESSAGE_CONTENT_TYPE_MIX_FILE_TEXT" />
        <UnsupportMessageContentView
            :message="message"
            v-else-if="
                [
                    /* todo un support message types */
                ].indexOf(message.messageContent.type) >= 0
            "
        />
...
import TextMessageContentView from '../../../main/conversation/message/content/TextMessageContentView.vue';
import ImageMessageContentView from '../../../main/conversation/message/content/ImageMessageContentView.vue';
import VideoMessageContentView from '../../../main/conversation/message/content/VideoMessageContentView.vue';
import UnsupportMessageContentView from '../../../main/conversation/message/content/UnsupportMessageContentView.vue';
import FileMessageContentView from '../../../main/conversation/message/content/FileMessageContentView.vue';
import StickerMessageContentView from '../../../main/conversation/message/content/StickerMessageContentView.vue';
import CallStartMessageContentView from '../../../main/conversation/message/content/CallStartMessageContentView.vue';
// 语音消息说明
//  目前提供连个实现版：
//  1. 基于APP_SERVER做编码转换，采用audio标签，播放mp3文件，对应AudioMessageContentViewAPP
//  2. 本地解码，采用自定义UI，直接播放AMR文件，对应AudioMessageContentViewAMR
import AudioMessageContentView from '../../../main/conversation/message/content/AudioMessageContentViewAMR';
// import AudioMessageContentView from "../../../main/conversation/message/content/AudioMessageContentViewAPP";
import CompositeMessageContentView from '../../../main/conversation/message/content/CompositeMessageContentView';
