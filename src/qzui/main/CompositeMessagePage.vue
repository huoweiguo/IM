<template>
    <section class="composite-page">
        ...
        <ul v-else>
            <li v-for="(message, index) in compositeMessage.messageContent.messages" :key="message.uid">
                <div class="message-container">
                    <div class="portrait-container">
                        <img v-if="index === 0 || message.from !== compositeMessage.messageContent.messages[index - 1].from" alt="" :src="message._from.portrait" />
                    </div>
                    <div class="name-time-content-container">
                        ...
                        <div class="content">
                            ...
                            <ImageMessageContentView :message="message" :is-in-composite-view="true" v-else-if="message.messageContent.type === 3" />
                            <!--                           v-bind:class="{leftarrow:message.direction === 1, rightarrow: message.direction === 0}"/>-->
                            <FileMessageContentView
                                ...
                                v-bind:class="{ leftarrow: message.direction === 1, rightarrow: message.direction === 0 }"
                            />
                            <VideoMessageContentView :message="message" :is-in-composite-view="true" v-else-if="message.messageContent.type === 6" />
                            <!--                           v-bind:class="{leftarrow:message.direction === 1, rightarrow: message.direction === 0}"/>-->
                            ...
                            <UnsupportMessageContentView :message="message" v-else-if="[2, 10, 400, 408].indexOf(message.messageContent.type) >= 0" />
                            <UnknowntMessageContentView :message="message" v-else v-bind:class="{ leftarrow: message.direction === 1, rightarrow: message.direction === 0 }" />
                        </div>
                    </div>
                </div>
            </li>
