...
const remoteWindow = (windowId) => {
  return {
    ...
    isAlwaysOnTop: () =>
      ipcRenderer.sendSync('window-state', { windowId, command: 'isAlwaysOnTop' }),
    getMediaSourceId: () =>
      ipcRenderer.sendSync('window-state', { windowId, command: 'getMediaSourceId' }),
    ...
    setOpacity: (opacity) =>
      ipcRenderer.send('window-control', { windowId, command: 'setOpacity', args: opacity }),
    setFullScreen: (flag) =>
      ipcRenderer.send('window-control', { windowId, command: 'setFullScreen', args: flag }),
    setAlwaysOnTop: (flag) =>
      ipcRenderer.send('window-control', { windowId, command: 'setAlwaysOnTop', args: flag }),
    setSize: (width, height, animate) =>
      ipcRenderer.send('window-control', {
        windowId,
        command: 'setSize',
        args: [width, height, animate]
      }),
    setMinimumSize: (width, height) =>
      ipcRenderer.send('window-control', {
        windowId,
        command: 'setMinimumSize',
        args: [width, height]
      }),
    setIgnoreMouseEvents: (ignore, options) =>
      ipcRenderer.send('window-control', {
        windowId,
        command: 'setIgnoreMouseEvents',
        args: [ignore, options]
      }),
    ...
    webContents: {
      emit: (event, ...args) =>
        ipcRenderer.send('webcontents-emit', { windowId, channel: event, args }),
      // on: (channel, listener) => {
      //     ipcRenderer.on(`window-${windowId}-webcontents-${channel}`, listener)
      // },
      send: (channel, ...args) =>
        ipcRenderer.send('webcontents-send', { windowId, channel: channel, args }),
      openDevTools: () => ipcRenderer.send('webcontents-openDevTools', { windowId }),
      setZoomFactor: (factor) =>
        ipcRenderer.send('webcontents-setZoomFactor', { windowId, args: factor })
    }
  }
}
