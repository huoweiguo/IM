...
class WfcImpl {
  ...
  onConnectionChanged(status) {
    ...
    if (status === ConnectionStatus.ConnectionStatusConnected) {
      if (self.isEnableSecretChat() && self.isUserEnableSecretChat()) {
        ipcRenderer.send('start-secret-server', {
          port: Config.SECRET_CHAT_MEDIA_DECODE_SERVER_PORT
        })
      }
  ...
  connect(userId, token) {
    ...
    if (!self.userId) {
      ...
      if (
        status !== ConnectionStatus.ConnectionStatusConnecting &&
  ...
  async sendMessageEx(message, toUsers = [], preparedCB, progressCB, successCB, failCB) {
    ...
    if (
      ...
      if (
        ...
        (!message.messageContent.file || !(message.messageContent.file instanceof File))
      ) {
      ...
      if (
        ...
        if (message.messageContent.localPath) {
          let thumbnail = await require('electron').nativeImage.createThumbnailFromPath(
            message.messageContent.localPath,
  ...
  async _uploadFile(message, file, mediaType, progressCB, successCB, failCB) {
    ...
    if (message && message.conversation.type === ConversationType.SecretChat) {
      ...
      let encodedAb = protoProxy.invoke(
